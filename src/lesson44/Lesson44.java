...
public class Lesson44 {
    public static void main(String[] args) {
        File fileBin = new File("file.bin");
        System.out.println(fileBin.exists());

        //byte array input stream
        byte[] inputBuffer = {1, 2, 3, 4, 5, 6};//{65, 66, 67};
        ByteArrayInputStream is = new ByteArrayInputStream("Hə".getBytes());
        int x;
        while ((x = is.read()) != -1) {
            System.out.print(x + " ");
        }
        ...
        byte[] inputBufer = new byte[bufferSize];
        ...
        System.out.println(Arrays.toString(inputBufer));
        ...
        try {
            lines = Files.readAllLines(Paths.get("components"));
